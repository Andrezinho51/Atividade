...
class Scrapper {
    ...
    public function scrap(\DOMDocument $dom): array {
        ...
        //Retorna os nomes dos autores separados
        foreach ($a_elements as $a_element) {
        ...
        //Retorna os IDS de cada um dos artigos
        foreach ($a_elements as $a_element) {
            $div_elements = $a_element->getElementsByTagName('div');
            foreach ($div_elements as $div_element) {
        ...
        //Retorna os tipos de cada arquivo
        foreach ($a_elements as $a_element) {
            $div_elements = $a_element->getElementsByTagName('div');
            foreach ($div_elements as $div_element) {
        ...
        //Retorna os nomes de cada uma das instituiçoes
        foreach ($a_elements as $a_element) {
            $div_elements = $a_element->getElementsByTagName('div');
            foreach ($div_elements as $div_element) {
                if ($div_element->getAttribute('class') === 'authors') {
        ...
        for($i = 0; $i < $work_counter; $i++){
            $data_final[$i] = [];
            //id, titulo e tipo do projeto
            array_push($data_final[$i], $data_id[$i], $data_title[$i], $data_type[$i]);
            //for que adiciona os autores e suas instituicoes em seus devidos projetos
